 ...
 *
 * Event production is now pluggable: the engine can ingest simulated,
 * replayed, or future live events through the same policy pipeline.
 */
...
import { ThreatEventSource } from './eventSources';
...
const SOURCE_COUNTRIES = [
    { name: 'Russia', lat: 61.524, lon: 105.318 }, { name: 'China', lat: 35.861, lon: 104.195 },
    { name: 'North Korea', lat: 40.339, lon: 127.510 }, { name: 'Iran', lat: 32.427, lon: 53.688 },
    { name: 'Brazil', lat: -14.235, lon: -51.925 }, { name: 'Nigeria', lat: 9.082, lon: 8.675 },
    { name: 'Vietnam', lat: 14.058, lon: 108.277 }, { name: 'India', lat: 20.593, lon: 78.962 },
    { name: 'Pakistan', lat: 30.375, lon: 69.345 }, { name: 'Indonesia', lat: -0.789, lon: 113.921 },
    { name: 'Turkey', lat: 38.963, lon: 35.243 }, { name: 'Romania', lat: 45.943, lon: 24.966 },
...
export class DefenseSystem {
    ...
    private auditHandle: ReturnType<typeof setInterval> | null = null;
    private eventSource: ThreatEventSource | null = null;
    private threatSeq = 0;
    private startedAt = Date.now();
    private stats: DefenseStats = { total: 0, neutralized: 0, byCategory: {}, bySeverity: { low: 0, medium: 0, high: 0, critical: 0 }, recentRate: 0, uptimeMs: 0, lastTick: 0 };
    private recentWindow: number[] = [];
    private context: DefenseContext = { connected: false, config: {} as VPNConfig, userLocation: null };
    private paused = false;

    /** Attach a simulation, replay, or future live source. The source only produces events; this engine evaluates them. */
    setEventSource(source: ThreatEventSource | null): void {
        if (this.eventSource) this.eventSource.stop();
        this.eventSource = source;
        if (source && this.tickHandle) source.start(event => this.ingest(event));
    }
    ...
    start(): void {
        ...
        if (this.eventSource) {
            this.eventSource.start(event => this.ingest(event));
        } else {
            this.tickHandle = setInterval(() => this.tick(), 1500);
        }
        this.feedHandle = setInterval(() => this.emitFeed(), 5000);
        this.auditHandle = setInterval(() => saveAudit(this.audit), 4000);
        this.emitStats();
    }
    ...
    setContext(ctx: Partial<DefenseContext>): void { this.context = { ...this.context, ...ctx }; this.contextListeners.emit(this.context); }
    setPaused(p: boolean): void { this.paused = p; }
    isPaused(): boolean { return this.paused; }

    getRules(): DefenseRule[] { return [...this.rules]; }
    setRules(rules: DefenseRule[]): void { this.rules = rules; saveRules(rules); this.ruleListeners.emit(this.rules); this.appendAudit('RULES_UPDATED', `Rule set replaced (${rules.length} rules)`, 'system'); }
    upsertRule(rule: DefenseRule): void {
        const idx = this.rules.findIndex(r => r.id === rule.id);
        if (idx >= 0) this.rules[idx] = rule; else this.rules.push(rule);
        saveRules(this.rules); this.ruleListeners.emit(this.rules);
        this.appendAudit('RULE_UPSERT', `${rule.enabled ? 'Enabled' : 'Disabled'} rule "${rule.name}"`, 'system');
    }
    deleteRule(id: string): void {
        const before = this.rules.length; this.rules = this.rules.filter(r => r.id !== id);
        if (this.rules.length !== before) { saveRules(this.rules); this.ruleListeners.emit(this.rules); this.appendAudit('RULE_DELETED', `Rule ${id} removed`, 'system'); }
    }
    toggleRule(id: string): void {
        const r = this.rules.find(r => r.id === id); if (!r) return;
        r.enabled = !r.enabled; saveRules(this.rules); this.ruleListeners.emit(this.rules);
        this.appendAudit('RULE_TOGGLED', `${r.enabled ? 'Enabled' : 'Disabled'} rule "${r.name}"`, 'system');
    }
    resetRules(): void { this.rules = DEFAULT_RULES; saveRules(this.rules); this.ruleListeners.emit(this.rules); this.appendAudit('RULES_RESET', 'Rules reset to defaults', 'system'); }
    ...
    neutralize(threatId: number, action: RuleAction = 'block'): { ok: boolean; ruleApplied?: DefenseRule } {
        ...
        this.appendAudit('MANUAL_NEUTRALIZE', `Manual neutralize: action=${action}, threat=${threatId}`, 'user', threatId);
        return { ok: true, ruleApplied: matchingRule };
    }

    blockDomain(domain: string): void {
        const rule: DefenseRule = { id: `user-domain-${Date.now()}`, name: `Block ${domain}`, enabled: true, category: 'all', severity: 'low', action: 'block', source: 'user', createdAt: Date.now() };
        this.upsertRule(rule); this.appendAudit('DOMAIN_BLOCKED', `Added block rule for ${domain}`, 'user');
    }

    /** Public ingestion seam shared by simulation, replay and future adapters. */
    ingest(ev: ThreatEvent, options: { immediate?: boolean } = {}): void {
        if (this.paused) return;
        this.evaluate(ev, options.immediate === true);
    }

    /** Generates a synthetic event for SimulationEventSource; policy remains in ingest(). */
    generateThreat(): ThreatEvent { return this.spawnThreat(); }

    private tick(): void {
        if (this.paused) return;
        const now = Date.now();
        this.stats.uptimeMs = now - this.startedAt;
        ...
        for (let i = 0; i < threatsPerTick; i++) this.ingest(this.spawnThreat());
    ...
    private spawnThreat(): ThreatEvent {
        ...
        if (Math.random() < 0.35) {
            ...
            return { id, category: 'RF', type: band.kind, subType: band.name, severity: band.severity, coords: { lat: userLoc.lat + offset, lon: userLoc.lon + offset }, timestamp: t, status: 'detecting', signal: { frequency: `${freq.toFixed(band.unit === 'kHz' ? 1 : 3)} ${band.unit}`, powerDbm: `${(-90 + Math.random() * 60).toFixed(1)} dBm`, distance: `${(50 + Math.random() * 450).toFixed(0)} m` } };
        }
        ...
        return { id, category: 'CYBER', type: cat.name, severity: cat.severity, coords: { lat: source.lat, lon: source.lon }, timestamp: t, status: 'detecting', cyber: { sourceCountry: source.name, sourceIp: `${randomOctet()}.${randomOctet()}.${randomOctet()}.${randomOctet()}`, asn: asn.asn, asnName: asn.name, domain: `${randomLabel()}.${domain}`, protocol: ['TCP', 'UDP', 'QUIC', 'DNS'][Math.floor(Math.random() * 4)] as 'TCP' | 'UDP' | 'QUIC' | 'DNS', destinationPort: 80 + Math.floor(Math.random() * 8000) } };
    }

    private evaluate(ev: ThreatEvent, immediate = false): void {
        this.stats.total += 1;
        ...
        ev.status = 'locking'; ev.ruleApplied = rule.id; ev.responsePlan = rule.action;
        this.threatListeners.emit(ev);
        if (immediate) {
            ev.status = 'neutralizing'; this.threatListeners.emit(ev);
            ev.status = 'neutralized'; ev.counterMeasure = describeAction(rule.action, ev); this.threatListeners.emit(ev);
    ...
    private emitFeed(): void {
        this.feedListeners.emit({ generatedAt: Date.now(), sources: SUSPICIOUS_ASN.map(s => ({ asn: s.asn, name: s.name, cc: s.cc, events: Math.floor(Math.random() * 200), reputation: Math.max(0.1, Math.random()) })), globalStats: { ...this.stats.byCategory } as Record<string, number> });
    }
    private emitStats(): void { this.statsListeners.emit({ ...this.stats }); }
    private appendAudit(event: string, details: string, actor: 'auto' | 'user' | 'system', threatId?: number): void {
        const entry: AuditEntry = { id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, timestamp: Date.now(), event, details, actor, threatId };
        this.audit.push(entry); if (this.audit.length > MAX_AUDIT) this.audit = this.audit.slice(-MAX_AUDIT); this.auditListeners.emit(entry);
    }
}
...
const weightedPick = <T,>(items: Array<{ v: T; w: number }>): T => {
    const total = items.reduce((s, i) => s + i.w, 0); let r = Math.random() * total;
...
const matchesRule = (rule: DefenseRule, ev: ThreatEvent): boolean => {
    ...
    if (rule.matchCountries && ev.cyber?.sourceCountry && !rule.matchCountries.includes(ev.cyber.sourceCountry)) return false;
    if (rule.matchAsn && ev.cyber?.asn && !rule.matchAsn.includes(ev.cyber.asn)) return false;
    if (rule.matchSubType && ev.subType && !rule.matchSubType.includes(ev.subType)) return false;
    return true;
};
